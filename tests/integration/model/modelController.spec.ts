...
describe('ModelController', function () {
  ...
  describe('POST /models (createModel)', function () {
    describe('Happy Path 🙂', function () {
      ...
      it('should return 201 status code and and pass unpublished status to store trigger if recieved publish', async function () {
        const payload = createIngestionPayload();
        payload.metadata.productStatus = RecordStatus.PUBLISHED;
        const storeTriggerResult: StoreTriggerResponse = {
          jobId: faker.string.uuid(),
          status: OperationStatus.IN_PROGRESS,
        };
        ...
        const subsetPostPayloadMetadata = {
          ...
          footprint: payload.metadata.footprint,
